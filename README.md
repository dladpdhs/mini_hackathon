...
여러 과목의 강의계획서를 분석해 한 학기 전체 일정(퀴즈·과제·시험·발표)을 하나의 캘린더로 통합하고,
마감이 몰리는 주를 미리 알려주는 프로그램입니다. 듣는 과목만 고르면 캘린더가 자동으로 만들어집니다.

## 바로 사용하기 (설치 불필요)

**https://mini-hackathon-xvp5.onrender.com**

위 주소로 접속하면 브라우저에서 바로 사용할 수 있습니다. 설치나 로그인이 필요 없습니다.
무료 호스팅이라 첫 접속 시 서버가 깨어나는 데 약 50초가 걸릴 수 있습니다.
직접 코드를 실행하려면 아래 **6. How to Run**을 참고하세요.

## 주요 기능

- 과목명을 검색·선택하면(교수·분반 단위) 일정이 자동으로 통합됩니다.
- 강의·과제·퀴즈·시험·발표를 월별 달력에 색으로 표시하고, 오늘 날짜를 강조합니다.
- 오늘 기준 2주 내 마감을 D-day로 상단에 표시합니다.
- 여러 과목의 마감·시험이 같은 주에 몰리면 '과부하 주차'로 경고합니다.
- 같은 과목도 교수·분반별로 구분해 선택할 수 있습니다. (예: 「컴퓨터의 개념 및 실습」 012/006/016)
- 통합 일정을 `.ics` 파일로 내보내 구글·애플 캘린더로 가져갈 수 있습니다.
- 새 강의계획서 PDF를 올리면 분석해 과목을 추가합니다. (API 키 없이 내장 30여 과목으로 동작)

## 화면

**1) 과목 선택** — 과목명·교수로 검색해서 교수·분반을 선택합니다.

![과목 선택 화면](docs/screen_select.png)

**2) 결과 상단** — 마감 임박(D-day) 알림과 과부하 주차 경고, 그리고 월별 달력.

![결과 상단 화면](docs/screen_result.png)

**3) 월별 달력** — 강의·과제·퀴즈·시험·발표가 실제 요일 칸에 과목별 색으로 표시됩니다.

![월별 달력 화면](docs/screen_calendar.png)
...
**가장 쉬운 방법 — 그냥 접속:** 👉 **https://mini-hackathon-xvp5.onrender.com** (설치 불필요)

아래는 코드를 직접 내려받아 로컬에서 실행하는 방법입니다.
